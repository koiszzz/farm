...
const OUTPUT_DIR := "res://design/qa/2026-09-08/movement-performance"
const SAMPLE_CELLS := [
...
func _measure_visible_walk() -> Dictionary:
	...
	game.pending_cell = route[0]
	game.player_body.position = game._avatar_position_for(route[0])
	game.move_target = game.player_body.position
	game.moving = false
	...
	for route_index in range(1, mini(route.size(), 49)):
		var direction: Vector2i = route[route_index] - game.player_cell
		if absi(direction.x) + absi(direction.y) != 1:
			continue
		game._begin_move(direction)
		var guard := 0
		var previous_tick := Time.get_ticks_usec()
		while game.moving and guard < 120:
			await process_frame
