...
func _init() -> void:
	...
	_expect(navigation.map_ids() == PackedStringArray(["cafe_interior", "clinic_interior", "farm_outdoor", "farmhouse_interior", "general_store_interior", "town_square"]), "all six authored outdoor and interior maps should be indexed")
	_expect(navigation.get_map_size("farm_outdoor") == Vector2i(40, 30), "farm dimensions should match design data")
	_expect(navigation.get_spawn("town_square") == Vector2i(42, 22), "town spawn should match the safe design arrival cell")
	...
	_expect(not navigation.is_walkable("farm_outdoor", Vector2i(10, 2)), "farmhouse should block movement")
	...
	_expect(arrival is Array and arrival.size() == 2 and int(arrival[0]) == 42 and int(arrival[1]) == 22, "farm exit arrival should preserve the safe JSON arrival cell")
