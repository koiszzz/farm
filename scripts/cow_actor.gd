...
func tick(delta: float, roam_cells: Array[Vector2i], minutes: int, weather: String) -> void:
	...
	if local_cell == target_cell and wait_time <= 0:
		step_index += 1
		var seed := absi(animal_id.hash()) + step_index * 23
		target_cell = roam_cells[posmod(seed, roam_cells.size())]
		wait_time = 1.4 + float(posmod(seed, 5)) * 0.4
	...
	if moving:
		...
		local_position = local_position.move_toward(target_position, 16.0 * delta)
		stride = fmod(stride + previous_position.distance_to(local_position) * 4.0 / 25.0, 4.0)
...
func _update_sprite() -> void:
	...
	_sprite.position.y -= 1.0 if moving and int(elapsed * 7.0) % 2 == 0 else 0.0


func _draw() -> void:
	draw_set_transform(Vector2.ZERO, 0, Vector2(1, 0.4))
	draw_circle(Vector2(0, 6), 15, Color(0.14, 0.11, 0.08, 0.26))
	draw_set_transform(Vector2.ZERO)
	if affection > 0:
		var p := Vector2(-4, -40 - sin(elapsed * 6.0) * 2.0)
