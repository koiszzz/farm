...
const COVER_SECONDS := 0.05
const BLACK_HOLD_SECONDS := 0.01
...
func play_gated(scene_swap: Callable, gate_signal: Signal, gate_is_pending: Callable = Callable(), reveal_center := Vector2(0.5, 0.5)) -> void:
	if running:
		return
	running = true
	overlay.visible = true
	...
	cover.tween_method(_set_blackness, 0.0, 1.0, COVER_SECONDS).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN)
	await cover.finished
	# The gate may finish during the cover animation. Check its live state before
	# awaiting the signal so a completion emitted in that overlap cannot be lost.
	if not gate_is_pending.is_valid() or bool(gate_is_pending.call()):
		await gate_signal
	if scene_swap.is_valid():
		scene_swap.call()
	await get_tree().create_timer(BLACK_HOLD_SECONDS).timeout
