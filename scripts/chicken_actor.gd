...
const TILE := 32.0
const CHICKEN_ART: Texture2D = preload("res://assets/art/runtime_generated/chicken_walk_v1.png")
...
func configure(id: String, index: int, offset: Vector2i) -> void:
	...
	if local_cell == Vector2i.ZERO:
		local_cell = Vector2i(49 + (index % 3) * 2, 28 + (index / 3) * 2)
...
func tick(delta: float, roam_cells: Array[Vector2i], minutes: int, weather: String) -> void:
	...
	wait_time -= delta
	if local_cell == target_cell and wait_time <= 0:
		step_index += 1
		var seed := absi(animal_id.hash()) + step_index * 17
		target_cell = roam_cells[posmod(seed, roam_cells.size())]
		wait_time = 0.8 + float(posmod(seed, 7)) * 0.18
	...
	if moving:
		...
		facing = "left" if absf(direction.x) > absf(direction.y) and direction.x < 0 else "right" if absf(direction.x) > absf(direction.y) else "up" if direction.y < 0 else "down"
		var previous_position := local_position
		local_position = local_position.move_toward(target_position, 26.0 * delta)
...
func _update_sprite() -> void:
	...
	var frame: Dictionary = Atlas.frame(CHICKEN_ART, Vector2i(4, 4), column, int(rows.get(facing, 0)))
	...
	_sprite.position.y -= 1.0 if moving and int(elapsed * 9.0) % 2 == 0 else 0.0
...
func _draw() -> void:
	...
	draw_circle(Vector2(0, 4), 9, Color(0.16, 0.13, 0.08, 0.24))
	draw_set_transform(Vector2.ZERO)
	if affection > 0:
		var p := Vector2(-4, -33 - sin(elapsed * 6.0) * 2.0)
