...
func _run() -> void:
	...
	await tap(KEY_I)
	expect(game.inventory_panel.detail_card.visible and game.inventory_panel.detail_title.text == "锄头", "physical I input opens selected item details")
	await tap(KEY_ESCAPE)
	expect(game.inventory_panel.visible and not game.inventory_panel.detail_card.visible, "first Escape closes item details")
	await tap(KEY_ESCAPE)
