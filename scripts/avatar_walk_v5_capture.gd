...
const OUTPUT_PATH := "res://design/qa/2026-09-24/avatar-walk-v5/live-cycle.png"
const RUN_OUTPUT_PATH := "res://design/qa/2026-09-24/avatar-walk-v5/live-run-cycle.png"
...
func _capture() -> void:
	...
	for row in directions.size():
		...
		for column in 8:
			...
			actors.append(actor)
	DirAccess.make_dir_recursive_absolute("res://design/qa/2026-09-24/avatar-walk-v5")
	await create_timer(0.3).timeout
	await RenderingServer.frame_post_draw
	var capture := root.get_texture().get_image()
	capture.save_png(OUTPUT_PATH)
	print("Captured v5 walk cycle: " + OUTPUT_PATH + " size=" + str(capture.get_size()))
	for actor in actors: actor.running = true
	await process_frame
	await RenderingServer.frame_post_draw
	var run_capture := root.get_texture().get_image()
	run_capture.save_png(RUN_OUTPUT_PATH)
	print("Captured v5 run cycle: " + RUN_OUTPUT_PATH + " size=" + str(run_capture.get_size()))
	quit()
